...
class Manager
{
	...
	{
		switch(vi.codec) {
			case codec_screenpressor: decoder =	new ScreenPressor(vi.X, vi.Y, vi.bpp);
#if msvc
			case codec_msvc16: decoder = new MSVideo1_16bit(vi.X, vi.Y);
			case codec_msvc8: decoder = new MSVideo1_8bit(vi.X, vi.Y, vi.palette);
#end
		}
